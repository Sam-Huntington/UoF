...
* 2013/05/31. Lenaig added the option "RenewInvestments" that allows/does not allow optimal new investments in technologies.
    ModelOption 'model options'
        / DiscreteInvest, ContinuousInvest, NGChillInvest, RenewInvestments, Sales, PVSales, NetMetering, InvestmentConst,  SwitchInvest,  StandbyOpt, VaryPrice,
          CHP, CO2Tax, MinimizeCO2, ZNEB, MultiObjective, DiscreteElecStorage, LS, DR, CentralChiller, GSHPAnnualBalance, FuelCellConstraint /
...
*2013/05/31. Lenaig added a subset years_counter for the reason below.
    ...
    HOURS           24 hours
        / 1 * 24 /
    ENDUSE          enduse
        / misc-electric-loads, cooling, refrigeration, space-heating, water-heating, naturalgas-only /
    SEASONS         two seasons
        / summer, winter /
...
*SH!!! - Added Parameters
    TotalElecBill               All period electricity purchases
    TotalMonthlyDemandCost      All period demand (kw) charges
    TotalStandbyCost            All period demand (kw) charges
    TotalDailyDemandCost        All period demand (kw) charges
    TotalTOUCost                All period demand (kw) charges
    TotalFixedCost      All period demand (kw) charges
    TotalContractCost      All period demand (kw) charges
    TotalLS             Total kWh shifted up and down (so twice the load reduction)
    TotalPV             Total PV generation
    TotalPVSales        Total PV sales ($)
    ExportPV            Total PV Exports (kWh)
    OnsitePV            Total PV consumed Onsite (kWh)
    Total_LS_Costs      total LS Costs ($)
    Total_Energy_Costs  Total Energy Costs
*    LoadShiftInWindow(hours, enduse)    Total load shifted within a defined window of hours (e.g. after 10am)
;
* ----------------------------
...
*SH!! - moved parameter table to excel input workbook
$ontext
...
FC-HX-small-20-wSGIP   100             10              5778            0               .033            100             0               4               1               .40             1               1               1               0               0               0
FC-HX-med-20-wSGIP__   250             10              4629            0               .033            250             0               4               1               .40             1               1               1               0               0               0
...
*Dev Note: when adding input tables from excel file, do the following:
*   r# = exceltableX                                     [excel named range]
*   o# = exceltableX                                     [excel named range]
*   table gamstableX(parameter x, parameter y, etc.)    [GAMS name]
*   $include gamstableX

file TMP / tmp.txt /
$onecho  > tmp.txt
   i="UoF Test Data.xlsm"
   r1=AnnualLoad1
   o1=AnnualLoad1
   r2=PX_Prices
   o2=PX1
   ...
   r7=FuelPrice1
   o7=FuelPrice1
   r8=SchedulableLoadParameterTable
   o8=SchedulableLoadParameterTable
   r9=ParameterTable
   o9=ParameterTable
   r10=MonthlyFee1
   o10=MonthlyFee1
...
*SH!!!
table SchedulableLoadParameterTable (SLParameterOption, EndUse, SLParameterValue)
$include  SchedulableLoadParameterTable
;

table ParameterTable (ParameterOption, ParameterDummy)
$include  ParameterTable
;

table MonthlyFee1 (Service, ParameterDummy)
$include  MonthlyFee1
;

*table DemandResponseParameters1 (DemandResponseType,DemandResponseInvestParameters)
*$include  DemandResponseParameters1
*;
...
$include FuelPrice1
;
execute 'del tmp.txt AnnualLoad1 PX1 ElectricityRates11 SolarInsolation AmbientHourlyTemperature MonthlyFee1 MonthlyDemandRates1 FuelPrice1 SchedulableLoadParameterTable ParameterTable' ;
;
...
Parameter Fixedcomp     Fixed component on the tariff               [€]   /5068/  ;
...
Parameter Capacitypayment Payment for capcity OM and OS             [€]   /0.01/;
...
$offtext
Parameter PX (years,months,daytypes,hours);
PX (years,months,daytypes,hours) = (PX1(months,daytypes,hours))*Exchangerate*(1+Taxes+Elect_tax);
Parameter Load (years, enduse, months, daytypes, hours);
Load (years, enduse, months, daytypes, hours)= AnnualLoad1 (enduse, months, daytypes, hours);

*SH!!! - Max Electric load: needs to equal sum of end use loads
parameter max_electric_load;
max_electric_load = smax ((years, months, daytypes, hours), load(years, 'misc-electric-loads', months, daytypes, hours) + load(years, 'cooling', months, daytypes, hours)+ load(years, 'refrigeration', months, daytypes, hours));
...
MonthlyFee (years,'UtilElectric') = MonthlyFee1 ('UtilElectric','ParameterValue');
MonthlyFee (years,'UtilNGbasic')  = MonthlyFee1 ('UtilNGbasic','ParameterValue');
MonthlyFee (years,'UtilNGforDG')  = MonthlyFee1 ('UtilNGforDG','ParameterValue');
MonthlyFee (years,'UtilNGforABS') = MonthlyFee1 ('UtilNGforABS','ParameterValue');
MonthlyFee (years,'UtilDiesel')   = MonthlyFee1 ('UtilDiesel','ParameterValue');
***
...
PercentageSchedulablePeak      30
PercentageSchedulableWeek      30
PercentageSchedulableWeekend   30
MaxLoadInHour                  50
MaxIncrease                    50
;
...
SchedulableLoadParameterTable(SLParameterOption,EndUse,SLParameterValue) $ (OptionsTable ('LS','OptionValue')=0)=0;


*SH!!!!
*   Dev Note: added 'enduse' as a dimension to the MaxShiftableLoad parameter
*           added code to assign MaxShiftableLoad parameter by enduse
*
* Load Scheduling
*2013/03. Lenaig added a year index.
parameter MaxShiftableLoad (enduse, years,months, daytypes) days of each type;

    MaxShiftableLoad(enduse,years,months,'week') = sum(hours, Load(years,enduse,months,'week',hours))*SchedulableLoadParameterTable ('PercentageSchedulableWeek',enduse,'ParameterValue')/100;
    MaxShiftableLoad(enduse,years,months,'weekend') = sum(hours, Load(years,enduse,months,'weekend',hours))*SchedulableLoadParameterTable ('PercentageSchedulableWeekend',enduse,'ParameterValue')/100;
    MaxShiftableLoad(enduse,years,months,'peak') = sum(hours, Load(years,enduse,months,'peak',hours))*SchedulableLoadParameterTable ('PercentageSchedulablePeak',enduse,'ParameterValue')/100;
...
*SH!!! - TO DO: subtract load shifting from this
 ContractDemand (years)
        =
       smax ( (months,daytypes,hours), load(years,'misc-electric-loads',months,daytypes,hours)
...
*ElecConsumed is sum of end use loads, plus any used for storage
*ElecProvided is the sum of produced on site, plus any remaining energy that needs to be purchased form the grid
*ElecPurchased is effectivly the net generation minus ElecConsumed
...
Positive VARIABLES
    ...
    Electricity_PV_Export (years,daytypes,months,hours)                Electricity sales from PV      (kW)
    Electricity_PV_Onsite (years,daytypes,months,hours)                Electricity from PV for onsite use     (kW)
...
positive Variable LoadReduction (enduse,years,months,daytypes,hours);
positive Variable LoadIncrease (enduse,years,months,daytypes,hours);
Positive Variable Annual_LS_Costs(years);
Binary Variable ReductionOrAddition(enduse,years,months,daytypes,hours);
...
ContractCapacity(years,months)
        =G=
        load(years,'misc-electric-loads',months,daytypes,hours)
        + load(years,'cooling',months,daytypes,hours)
        + load(years,'refrigeration',months,daytypes,hours)
        - Electricity_Photovoltaics(years,months,daytypes,hours)
*        - net battery load
        - sum(enduse, loadreduction(enduse,years,months,daytypes,hours))
        + sum(enduse, loadincrease(enduse,years,months,daytypes,hours))
*        + load additions
...
Electricity_PV_Export.up(years, daytypes, months, hours)$((OptionsTable('Sales','OptionValue') eq 0) or (OptionsTable('PVSales','OptionValue') eq 0)) = 0;
...
*SH!!!
*2015/1/25  CHECK THAT LS WORKS HERE
...
Equation ElectricTotalCost_Eq(years,months);
     ...
     ElectricVehicleBilling(years,months) +
     EVHomeElectricityCO2Cost(years,months)
*    + sum(hours, loadreduction(enduse, years,months,daytypes,hours)
     ;
...
*SH!!!
*2015/1/25  TO DO: Add cost parameter from LS
...
*SH!!!! add LS costs
         +
         DERCost(years) + DemandResponseCosts(years)  +  DemandResponseCostsHeating(years) + Annual_LS_Costs(years)
...
Parameters ElectricNonCoolingLoad(years,months,daytypes,hours);
     ElectricNonCoolingLoad(years,months,daytypes,hours) = Load(years,'misc-electric-loads',months,daytypes,hours);

Positive Variables
     ElectricityConsumed(years,daytypes,months,hours)
...
*SH!!! - total elec consumption
*   Dev Note: summed across end use for LoadReduction/Increase
Equation ElectricityConsumedEq (years,daytypes,months,hours);
     ElectricityConsumedEq (years,daytypes,months,hours)..
     ElectricityConsumed(years,daytypes,months,hours)
     =E=
...
Equation ElectricSalesEq (years,months,daytypes,hours);
     ...
     ElectricSales(years,months,daytypes,hours)
     =E=
     Electricity_PV_Export(years,daytypes,months,hours)
...
Equation ElectricityBalanceEq (years,months,daytypes,hours);
     ...
     ElectricityProvided(years,months,daytypes,hours)
     =E=
     ElectricityConsumed(years,daytypes,months,hours) +  ElectricSales(years,months,daytypes,hours);
...
Equation Electricity_PhotovoltaicsEq(years,months,daytypes,hours);
     ...
     Electricity_Photovoltaics(years,months,daytypes,hours)
     =E=
     Electricity_PV_Export(years,daytypes,months,hours) + Electricity_PV_Onsite(years,daytypes,months,hours);
...
Equation ElectricityStorageStationaryConstraint2Eq(years,months,daytypes,hours);
     ...
     ElectricityStoredStationary(years,months,daytypes,hours)
     =G=
     ElectricityStorageStationaryCapacity(years,months)*ElectricityStorageStationaryParameter('MaxDepthOfDischarge');
...
*SH!!!!
*   Dev Note: added enduse dimension to all applicable paramaters
...
Equation Annual_LS_Costs_Eq(years);
        Annual_LS_Costs_Eq(years)..
        Annual_LS_Costs(years)
        =E=
        sum((enduse,months,daytypes,hours), LoadReduction(enduse,years,months,daytypes,hours))*0.0;

Equation LoadSchedulingEq_1(enduse,years,months,daytypes,hours);
     LoadSchedulingEq_1(enduse,years,months,daytypes,hours)..
     LoadReduction(enduse,years,months,daytypes,hours)
     =L=
     ReductionOrAddition(enduse,years,months,daytypes,hours)*SchedulableLoadParameterTable('MaxLoadinHour',enduse,'ParameterValue');

Equation LoadSchedulingEq_2(enduse,years,months,daytypes,hours);
     LoadSchedulingEq_2(enduse,years,months,daytypes,hours)..
     LoadIncrease(enduse,years,months,daytypes,hours)
     =L=
     (1-ReductionOrAddition(enduse,years,months,daytypes,hours))* SchedulableLoadParameterTable('MaxIncrease',enduse,'ParameterValue');

Equation LoadSchedulingEq_3(enduse,years,months,daytypes);
     LoadSchedulingEq_3(enduse,years,months,daytypes)..
     sum(hours, LoadReduction(enduse,years,months,daytypes,hours))
     =L=
     MaxShiftableLoad(enduse,years,months,daytypes);

Equation LoadSchedulingEq_4 (enduse,years,months,daytypes);
         LoadSchedulingEq_4 (enduse,years,months,daytypes)..
         sum(hours,LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours,LoadIncrease(enduse,years,months,daytypes,hours));

$ontext
Equation LoadSchedulingEq_5 (enduse,years,months,daytypes);
         LoadSchedulingEq_5  (enduse,years,months,daytypes)..
         sum(hours$(ord(hours)>=12 and ord(hours)<=15),LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours$(ord(hours)>=12 and ord(hours)<=15),LoadIncrease(enduse,years,months,daytypes,hours));


Equation LoadSchedulingEq_6 (enduse,years,months,daytypes);
         LoadSchedulingEq_6(enduse,years,months,daytypes)..
         sum(hours$(ord(hours)>=13 and ord(hours)<=16),LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours$(ord(hours)>=13 and ord(hours)<=16),LoadIncrease(enduse,years,months,daytypes,hours));


Equation LoadSchedulingEq_7 (enduse,years,months,daytypes);
         LoadSchedulingEq_7 (enduse,years,months,daytypes)..
         sum(hours$(ord(hours)>=14 and ord(hours)<=17),LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours$(ord(hours)>=14 and ord(hours)<=17),LoadIncrease(enduse,years,months,daytypes,hours));

Equation LoadSchedulingEq_8 (enduse,years,months,daytypes);
         LoadSchedulingEq_8 (enduse,years,months,daytypes)..
         sum(hours$(ord(hours)>=15 and ord(hours)<=18),LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours$(ord(hours)>=15 and ord(hours)<=18),LoadIncrease(enduse,years,months,daytypes,hours));

Equation LoadSchedulingEq_9 (enduse,years,months,daytypes);
         LoadSchedulingEq_9(enduse,years,months,daytypes)..
         sum(hours$(ord(hours)>=16 and ord(hours)<=19),LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours$(ord(hours)>=16 and ord(hours)<=19),LoadIncrease(enduse,years,months,daytypes,hours));

Equation LoadSchedulingEq_10 (enduse,years,months,daytypes);
         LoadSchedulingEq_10(enduse,years,months,daytypes)..
         sum(hours$(ord(hours)>=17 and ord(hours)<=20),LoadReduction(enduse,years,months,daytypes,hours))
         =e=sum(hours$(ord(hours)>=17 and ord(hours)<=20),LoadIncrease(enduse,years,months,daytypes,hours));
$offtext

*equation LS_CoolingConstraint;
*         LS_CoolingConstraint..
*         sum(hours$(hours>10 and hours<24),Loadincrease(enduse,years,months,daytypes,hours))-sum(hours$(hours>10 and hours<24),Loadreduction(enduse,years,months,daytypes,hours))=E=0;
*         sum(hours,Loadincrease(enduse,years,months,daytypes,hours))-sum(hours,Loadreduction(enduse,years,months,daytypes,hours))=E=0;
...
Electricity_PV_Export.fx(years,daytypes,months,hours)$((OptionsTable('RenewInvestments','OptionValue')eq 0) And(years.val gt ContinuousInvestParameter('PV','lifetime')) ) = 0;
Electricity_PV_Onsite.fx(years,daytypes,months,hours)$((OptionsTable('RenewInvestments','OptionValue')eq 0) And(years.val gt ContinuousInvestParameter('PV','lifetime')) ) = 0;
...
DemandResponse(DemandResponseType,years,months,daytypes,hours)=l= DemandResponseParameters(years,DemandResponseType,'MaxContribution')*Load(years,'misc-electric-loads',months,daytypes,hours);
...
AllPeriodElectD = sum((years,months,daytypes,hours),load(years,'misc-electric-loads',months,daytypes,hours)*NumberOfDays(months,daytypes));
...
*SH!!!
*   DevNote: new parameters
TotalElecBill = sum((years,months),ElectricTotalCost.l(years,months));
TotalMonthlyDemandCost = sum((years,months),ElectricDemandCost.l(years,months));
TotalFixedCost = sum((years,months),ElectricFixedCost.l(years,months));
TotalStandByCost = sum((years,months),StandbyCost.l(years,months));
TotalDailyDemandCost = sum((years,months),ElectricDailyDemandCost.l(years,months));
TotalTOUCost = sum((years,months),ElectricTOUCost.l(years,months));
TotalContractCost = sum((years,months),ContractCost.l(years,months));
TotalLS = sum((enduse,years,months,daytypes,hours),Loadreduction.l(enduse,years,months,daytypes,hours));
TotalPV = sum((years,months,daytypes,hours),Electricity_Photovoltaics.l(years,months,daytypes,hours));
TotalPVSales = sum((years),AnnualElectricitySales.l(years));
OnsitePV = sum((years, months, daytypes, hours), Electricity_PV_Onsite.l(years,daytypes,months,hours));
ExportPV = sum((years, months, daytypes, hours), Electricity_PV_Export.l(years,daytypes,months,hours));
Total_LS_Costs = sum(years,Annual_LS_Costs.l(years));
Total_Energy_Costs = sum(years,TotalEnergyCosts.l(years));

*CoolingByAbsorption(years,months,daytypes,hours)
*CoolingByNGChill(years,months,daytypes,hours)
*CoolingByHeatPumps(years,months,daytypes,hours)
*ElectricityForCooling
*ElectricityConsumed
*ElectricTOUCost
*ElectricTotalCost
*DemandResponseOnOff(DemandResponseType,years,months,daytypes,hours)
*ContractCost(years,months) +
*     ElectricFixedCost(years,months) +
*     StandbyCost(years,months) +
*     ElectricTOUCost(years,months) +
...
*NEW OUTPUTS PROCEDURE
*create worksheet in 'Outputs' wkb and 'UoF' wkb
*assign var/par to worksheet in put command
*add var/par to execute unload command


*SH!!!! - New outputs
*$ontext

File TMP2 /C:\Users\huntington\Documents\gamsdir\projdir\tmp2.txt/;
put TMP2
*load decrease, load increase, and total load
Put 'var=loadreduction rdim=4 rng=LoadShift!a1' /
put 'var=LoadIncrease rdim=4 rng=LoadInc!a1' /
put 'var=ElectricityConsumed rdim=3 rng=ElecLoad!a1'/
put 'var=Electricity_PV_Onsite rdim=3 rng=Solar!a1' / 'var=Electricity_PV_Export rdim=3 rng=Solar!a40' /
Put 'var=ElectricTotalCost rdim=2 rng=ElecTotalCost!a1'/
put 'par=PX rdim=3 rng=ElecTOUCost!a1'/
put 'par=TotalElecBill rng=Savings!a1' /'par=TotalTOUCost rng=Savings!a2'/'par=TotalMonthlyDemandCost rng=Savings!a3' / 'par=TotalFixedCost rng=Savings!a4' /
put 'par=TotalDailyDemandCost rng=Savings!a5' /'par=TotalStandbyCost rng=Savings!a6' / 'par=TotalContractCost rng=Savings!a7'/ 'par=TotalPVSales rng=Savings!a8'/
putclose   'par=TotalLS rng=Savings!a10'/ 'par=TotalPV rng=Savings!a11'/  'par=OnsitePV rng=Savings!a12'/ 'par=ExportPV rng=Savings!a13'/ 'par=Total_LS_Costs rng=Savings!a14'/ 'par=Total_Energy_Costs rng=Savings!a15'/


execute_unload 'tmp.gdx' TotalLS TotalPV TotalPVSales OnsitePV ExportPV Total_LS_Costs Total_Energy_Costs LoadReduction LoadIncrease ElectricityConsumed MaxShiftableLoad TotalElecBill ElectricTOUCost UpfrontCapitalCost ElectricTotalCost ContractCapacity TotalContractCost TotalTOUCost TotalFixedCost TotalStandbyCost TotalMonthlyDemandCost TotalDailyDemandCost Electricity_PV_Export Electricity_PV_Onsite PX

*save variables/parameters to gdx file
execute          'gdxxrw.exe tmp.gdx SQ=n EpsOut=0 O="UoF Outputs.xlsx" @tmp2.txt'
*outoputs to excel using commands in text file
execute          'del        tmp.gdx                             tmp2.txt'
...
$ontext
*SH!!! test outputs
file outputfile1 /C:\Users\huntington\Documents\gamsdir\projdir\LoadShift.csv/ ;
outputfile1.pc = 5;
outputfile1.pw = 255;
put outputfile1;
put 'enduse','years','months','daytypes','hours','kWh'; put /;
loop((enduse,years,months,daytypes,hours),
    put enduse.tl,years.tl,months.tl,daytypes.tl,hours.tl,loadreduction.l(enduse,years,months,daytypes,hours)/
);
putclose;


file outputfile2 /C:\Users\huntington\Documents\gamsdir\projdir\ElecLoad.csv/ ;
outputfile2.pc = 5;
outputfile2.pw = 255;
put outputfile2;
put 'years','months','daytypes','hours','kWh'; put /;
loop((years,months,daytypes,hours),
    put years.tl,months.tl,daytypes.tl,hours.tl,ElectricityConsumedEq.l(years,months,daytypes,hours)/
);
putclose;
...
file results /C:\Users\huntington\Documents\gamsdir\projdir\results.csv/ ;
...
loop (daytypes, put '',daytypes.tl /;
      loop (months, put months.tl ;
            loop (hours, put  (LoadReduction.l(enduse,years,months,daytypes,hours))
                 ) ;
            put /;
            );
     );
...
loop (daytypes, put '',daytypes.tl /;
      loop (months, put months.tl ;
            loop (hours, put  (Loadincrease.l(enduse,years,months,daytypes,hours))
                 ) ;
            put /;
            );
     );
