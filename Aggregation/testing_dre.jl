...
iHourlyInputs = readtable("$path/Hourly_Parameters.csv",header=true) #df1 #
iTechParameters = readtable("$path/Tech_Parameters.csv",header=true) #df2 #
...
for t = 1:T
  if pHP_Month[pMonth[t]] #if HP is turned on for the given month
      ...
      #@addConstraint(dre, sExtLosses[t]==pSetpoint[t])
  end
...
@defExpr(TotalCost, EnergyCost+NetworkCost+PeakDayCost)
...
rename!(dfUsage, {:x1=>:Month,:x2=>:Week,:x3=>:Hour,:x4=>:Buy_Energy,:x5=>:Sell_Energy,:x6=>:Power_Consumed,
                  ...
                  :x28=>:Monthly_Peak_Demand}
        )

aCosts = [getValue(TotalCost),getValue(EnergyCost),getValue(NetworkCost),getValue(PeakDayCost),getValue(TotalRevenue),getValue(EnergyRevenue),getValue(PrimaryReserveRevenue)]
writetable("$path"*"test_outputs.csv",dfUsage)